...
// .claude/_skills/using-mtcute/tools/get-class.ts
...
function resolveMtcuteCoreDir() {
  try {
    ...
    const sourceCoreDir = join(process.cwd(), "packages/core/src");
    if (existsSync(join(sourceCoreDir, "highlevel/types"))) {
      return sourceCoreDir;
    }
    throw new Error("Could not find @mtcute/core in node_modules or packages/core/src. Is mtcute installed?");
  }
}
...
async function collectDtsFiles(dir) {
  ...
  for (const entry of entries) {
    ...
    } else if ((entry.name.endsWith(".d.ts") || entry.name.endsWith(".ts")) && !entry.name.endsWith(".test.d.ts") && !entry.name.endsWith(".test.ts") && entry.name !== "index.d.ts" && entry.name !== "index.ts") {
      results.push(fullPath);
    }
  }
  return results;
}
function parseExports(content, file) {
  const exports = [];
...
var membersMode = !flags.has("--no-members");
if (!query && !showAll) {
...
var typesDir = join2(pkgDir, "highlevel", "types");
try {
  await stat(typesDir);
} catch {
...
var files = await collectDtsFiles(typesDir);
var allExports = [];
...
if (searchMode) {
  ...
  matches = allExports.filter(
    (e) => e.name.toLowerCase().includes(queryLower) || e.description.toLowerCase().includes(queryLower)
  );
  if (matches.length === 0) {
  ...
  if (matches.length === 0) {
    ...
    const descMatches = nameMatches.length === 0 ? allExports.filter((e) => e.description.toLowerCase().includes(queryLower)) : [];
