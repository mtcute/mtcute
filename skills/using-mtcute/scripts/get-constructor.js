...
function resolveMtcuteFile(subpath) {
  try {
    ...
    const sourceFile = join(process.cwd(), "packages/core/src", subpath);
    if (existsSync(sourceFile)) {
      return sourceFile;
    }
    throw new Error(`Could not find @mtcute file "${subpath}" in node_modules or packages/core/src. Is mtcute installed?`);
  }
}
