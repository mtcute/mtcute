...
import { ForgeCryptoProvider, ICryptoProvider, NodeCryptoProvider } from '../utils'

export function testCryptoProvider(c: ICryptoProvider): void {
    it('should calculate sha1', async () => {
        expect((await c.sha1(Buffer.from(''))).toString('hex')).to.eq('da39a3ee5e6b4b0d3255bfef95601890afd80709')
        expect((await c.sha1(Buffer.from('hello'))).toString('hex')).to.eq('aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d')
        expect((await c.sha1(Buffer.from('aebb1f', 'hex'))).toString('hex')).to.eq(
            '62849d15c5dea495916c5eea8dba5f9551288850',
        )
    })
    ...
    it('should calculate sha256', async () => {
        ...
        expect((await c.sha256(Buffer.from('aebb1f', 'hex'))).toString('hex')).to.eq(
    ...
    it('should calculate hmac-sha256', async () => {
        ...
        expect((await c.hmacSha256(Buffer.from(''), key)).toString('hex')).to.eq(
            '642711307c9e4437df09d6ebaa6bdc1b3a810c7f15c50fd1d0f8d7d5490f44dd',
        )
        expect((await c.hmacSha256(Buffer.from('hello'), key)).toString('hex')).to.eq(
            '39b00bab151f9868e6501655c580b5542954711181243474d46b894703b1c1c2',
        )
        expect((await c.hmacSha256(Buffer.from('aebb1f', 'hex'), key)).toString('hex')).to.eq(
            'a3a7273871808711cab17aba14f58e96f63f3ccfc5097d206f0f00ead2c3dd35',
        )
    })

    it('should derive pbkdf2 key', async () => {
        expect((await c.pbkdf2(Buffer.from('pbkdf2 test'), Buffer.from('some salt'), 10)).toString('hex')).to.eq(
    ...
    it('should encrypt and decrypt aes-ige', async () => {
        const aes = c.createAesIge(
            ...
            Buffer.from('6D656E746174696F6E206F6620494745206D6F646520666F72204F70656E5353', 'hex'),
        )
        expect(
            (
                await aes.encrypt(
                    Buffer.from('99706487a1cde613bc6de0b6f24b1c7aa448c8b9c3403e3467a8cad89340f53b', 'hex'),
                )
            ).toString('hex'),
        ...
        expect(
            (
                await aes.decrypt(
                    Buffer.from('792ea8ae577b1a66cb3bd92679b8030ca54ee631976bd3a04547fdcb4639fa69', 'hex'),
                )
            ).toString('hex'),
