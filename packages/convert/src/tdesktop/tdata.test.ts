...
if (import.meta.env.TEST_ENV === 'node') {
  describe('tdata', async () => {
    ...
    // crutch because vitest doesn't play well with dynamic .ts imports
    const nodeCrypto = await import('node:crypto')
    const { NodeCryptoProvider } = await import('@mtcute/node/utils.js')
    const crypto = getDefaultCryptoProviderImpl(nodeCrypto, NodeCryptoProvider)
